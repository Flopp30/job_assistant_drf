...
class Vacancy(CustomBaseModel):
    ...
    currency = models.ForeignKey(Сurrency, verbose_name='Currency', related_name='vacancys', default=1, on_delete=models.SET_DEFAULT)  # Вид валюты
    is_active = models.BooleanField(verbose_name='Active')  # Активна ли вакансия
    # Требования к кандидату
    candidate_experience_months = models.PositiveIntegerField(verbose_name="Candidate's experience (months)", default=0)  # Опыт работы кандидата (в месяцах)
    schedule = models.ManyToManyField(Schedule, verbose_name='Work schedule')  # График работы (полный день, полставки)
    employment = models.ManyToManyField(Employment, verbose_name='Employment')  # Вид работы (удаленка, стажировка, офис)
    ...
    professional_roles = models.CharField(verbose_name='Professional roles', max_length=50)  # Должность (начальник, повар)
    ...
    created_at = models.DateTimeField(auto_now=True, verbose_name='Created at')  # Время публикации вакансии (у нас)
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Updated at')  # Время создания вакансии (у нас)
